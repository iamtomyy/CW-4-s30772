...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = from emp in emps join mgr in emps on emp.Mgr equals mgr.EmpNo 
            select new
            {
                Employee = emp.EName,
                Manager = mgr.EName
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = from e in emps join d in depts on e.DeptNo equals d.DeptNo from g in grades where e.Sal >= g.Losal && e.Sal <= g.Hisal
            select new
            {
                EName = e.EName,
                DName = d.DName,
